...
import (
	...
	"github.com/amedumer/checkers/x/checkers/rules"
...
func (k msgServer) PlayMove(goCtx context.Context, msg *types.MsgPlayMove) (*types.MsgPlayMoveResponse, error) {
	...
	captured, moveErr := game.Move(rules.Pos{X: int(msg.FromX), Y: int(msg.FromY)}, rules.Pos{X: int(msg.ToX), Y: int(msg.ToY)})
	...
	return &types.MsgPlayMoveResponse{CapturedX: int32(captured.X), CapturedY: int32(captured.Y), Winner: rules.PieceStrings[game.Winner()]}, nil
}
